...
def register_html_plugin(tag_name, is_self_closing=False):
    ...
    def plugin(scope, arguments, block, classes, id_name):
        return output_element(scope, arguments, block, tag_name, classes, id_name, is_self_closing)
    plugin.__name__ = tag_name


from .tags import BLOCK_TAGS, INLINE_TAGS, SELF_CLOSING

for tag in BLOCK_TAGS + INLINE_TAGS:
